...
### 4. On-The-Fly 레이저 가공
가공물을 정지시키지 않고 컨베이어로 연속 이송하는 상태에서 레이저 스캐너로 마킹·가공을 수행하여, 정지-가공-이송을 반복하는 인덱싱 방식 대비 택트 타임을 크게 단축합니다. 엔코더가 이송량을 실시간 검출하고 스캐너 컨트롤러가 마킹 좌표를 연속 보정하므로, 가공물은 흐르고 있지만 정지한 대상을 가공하는 것과 동일한 정밀도를 얻습니다.

**2D On-The-Fly**
- 이송 동기화: 엔코더로 컨베이어 이동량을 검출하여 X·Y 갈바노 마킹 좌표를 평행 이동(offset) 보정합니다.
- 연속 라인 적용: 초점 거리가 고정된 평면 가공물의 컨베이어 마킹·절단 라인에서 정지 없이 고속 가공합니다.

**3D On-The-Fly — VCANUS 보유 기술**
- 3D 경로 생성: 가공물의 3D 표면 형상(높이 맵·CAD 모델)으로부터 곡면·단차를 따라가는 최적 가공 경로를 생성합니다.
...
| On-The-Fly 레이저 가공 | 무정지 연속 가공, 3D 경로 생성, 곡면 추종 실시간 초점 제어. |
